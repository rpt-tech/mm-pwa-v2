...
import { Package, Search, AlertCircle, MapPin, Phone } from 'lucide-react';
...
export default function TrackOrderPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['trackOrder', searchParams?.orderNumber, searchParams?.email],
    queryFn: () =>
      gqlClient.request(GET_GUEST_ORDER, {
  ...
  const order = (data as any)?.guestOrder;

  const onSubmit = (formData: TrackFormData) => setSearchParams(formData);

  return (
    <div className="container mx-auto px-4 py-6 max-w-2xl">
      <Helmet>
        <title>Theo dõi đơn hàng | MM Mega Market</title>
        <meta name="description" content="Theo dõi trạng thái đơn hàng của bạn tại MM Mega Market" />
      </Helmet>
      ...
      <p className="text-sm text-gray-500 mb-6">
        Nhập số đơn hàng và email để xem trạng thái giao hàng.
      </p>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          >
            <Search size={16} />
            {isLoading ? 'Đang tra cứu...' : 'Theo dõi đơn hàng'}
          </button>
        </form>
      </div>
