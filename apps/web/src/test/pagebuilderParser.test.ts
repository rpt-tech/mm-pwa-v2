...
describe('row', () => {
  it('parses a basic row', () => {
    ...
    expect(r[0].contentType).toBe('row');
    expect(r[0].appearance).toBe('contained');
  });
  ...
  it('extracts box props from style', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].marginTop).toBe('10px');
    expect(r[0].paddingBottom).toBe('20px');
  });
  ...
  it('filters pagebuilder- css classes', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].cssClasses).toContain('my-class');
    expect(r[0].cssClasses).not.toContain('pagebuilder-mobile-hidden');
  });
  ...
  it('parses nested column children', () => {
    ...
    expect(r[0].children![0].contentType).toBe('column');
  });
});
...
describe('column', () => {
  it('parses width from style', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('column');
    expect(r[0].width).toBe('33.3333%');
  });
  ...
  it('parses appearance', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].appearance).toBe('align-top');
  });
});
...
describe('banner', () => {
  it('parses desktop/mobile images from data-background-images', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('banner');
    expect(r[0].desktopImage).toBe('https://cdn.example.com/desktop.jpg');
    expect(r[0].mobileImage).toBe('https://cdn.example.com/mobile.jpg');
  });
  ...
  it('parses link and openInNewTab', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].link).toBe('/promo');
    expect(r[0].openInNewTab).toBe(true);
  });
  ...
  it('parses button text', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].showButton).toBe('always');
    expect(r[0].buttonText).toBe('Shop Now');
  });
  ...
  it('parses overlay color', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].showOverlay).toBe('hover');
    expect(r[0].overlayColor).toBe('rgba(0,0,0,0.5)');
  });
  ...
  it('defaults showOverlay to "never" when attribute absent', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].showOverlay).toBe('never');
  });
  ...
  it('handles malformed data-background-images gracefully', () => {
    ...
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('banner');
    expect(r[0].desktopImage).toBeUndefined();
  });
});
...
describe('slider', () => {
  it('parses autoplay and speed', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('slider');
    expect(r[0].autoplay).toBe(true);
    expect(r[0].autoplaySpeed).toBe(3000);
  });
  ...
  it('defaults autoplay to false', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].autoplay).toBe(false);
  });
  ...
  it('sets infinite=false when data-infinite-loop="false"', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].infinite).toBe(false);
  });
  ...
  it('parses slide children', () => {
    ...
    if (!result) return; const r = result!;
    expect(r[0].children).toHaveLength(1);
    expect(r[0].children![0].contentType).toBe('slide');
  });
});
...
describe('html content type', () => {
  it('decodes HTML entities in inner content', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('html');
    expect(r[0].html).toContain('<p>Hello & World</p>');
  });
  ...
  it('decodes &quot; entities', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].html).toContain('href="/test"');
  });
...
describe('heading', () => {
  it('parses h1 tag', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('heading');
    expect(r[0].headingType).toBe('h1');
    expect(r[0].text).toBe('Main Title');
  });
  ...
  it('parses h3 tag', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].headingType).toBe('h3');
  });
  ...
  it('defaults to h2 when no heading tag found', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].headingType).toBe('h2');
  });
});
...
describe('buttons', () => {
  it('parses buttons container', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('buttons');
    expect(r[0].isSameWidth).toBe(false);
  });
  ...
  it('sets isSameWidth=true for stacked appearance', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].isSameWidth).toBe(true);
  });
  ...
  it('parses button-item children', () => {
    ...
    if (!result) return; const r = result!;
    expect(r[0].children).toHaveLength(1);
    expect(r[0].children![0].contentType).toBe('button-item');
    expect(r[0].children![0].text).toBe('Sale');
  });
});
...
describe('button-item', () => {
  ...
  it('detects primary button type from class', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].buttonType).toBe('primary');
  });
  ...
  it('detects secondary button type from class', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].buttonType).toBe('secondary');
  });
...
describe('divider', () => {
  it('parses hr width and color', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('divider');
    expect(r[0].width).toBe('80%');
    expect(r[0].color).toBe('#ff0000');
    expect(r[0].thickness).toBe('2px');
  });
...
describe('block', () => {
  it('parses block-id from data attribute', () => {
    ...
    const result = parsePageBuilderHtml(html);
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('block');
    expect(r[0].blockId).toBe('42');
  });
...
describe('unknown content type fallback', () => {
  it('falls back to html contentType with outerHTML', () => {
    ...
    if (!result) return; const r = result!;
    expect(r[0].contentType).toBe('html');
    expect(r[0].html).toContain('data-content-type="custom-widget"');
  });
});
...
describe('___widget_flashsale shortcode parsing', () => {
  ...
  it('parses pageSize, url, and title', () => {
    ...
    expect(r.url).toBe('/deals');
    expect(r.title).toBe('Flash Sale');
  });
...
describe('___widget_product_recommendation shortcode parsing', () => {
  ...
  it('parses id, limit, color, image, imageMobile', () => {
    ...
    expect(r.asmJourneyId).toBe('journey-123');
    expect(r.pageSize).toBe(8);
    expect(r.color).toBe('#ff0000');
    expect(r.image).toBe('/img/banner.jpg');
    expect(r.imageMobile).toBe('/img/banner-m.jpg');
  });
  ...
  it('color/image/imageMobile are undefined when absent', () => {
    ...
    expect(r.imageMobile).toBeUndefined();
  });
...
describe('multiple top-level content types', () => {
  it('returns array with all top-level blocks', () => {
    ...
    expect(r[0].contentType).toBe('row');
    expect(r[1].contentType).toBe('row');
    expect(r[2].contentType).toBe('html');
  });
});
